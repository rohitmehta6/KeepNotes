...
const CreateNote = () => {
    ...
    const generateRandomColor = () => {
        const red = Math.floor((1 + Math.random()) * 125);
        const green = Math.floor((1 + Math.random()) * 125);
        const blue = Math.floor((1 + Math.random()) * 125);
    ...
    const addHandler = () => {
        if (input.title === "") {
            ...
            setNotes(edit);
            setDupNotes(edit);
            setInput({ title: "", content: "", backgroundColor: "" });
            setBtn(true);
            setItemToEdit(null);
        }
        else {
    ...
    const editHandler = (id) => {
        setInput({ title: notes[id].title, content: notes[id].content, backgroundColor: notes[id].backgroundColor });
        setBtn(false);
        setItemToEdit(id);
    }
            ...
            <div>
                ...
                <div className="all_notes">
                    ...
                    {notes.map((item, index) => {
                        return (
                            <div key={index} style={{ backgroundColor: item.backgroundColor }} id={index} className="note">
                                <h3>{item.title}</h3>
                                <p>{item.content}</p>
